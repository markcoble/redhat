...
for i in ${APP_HOSTS[@]}; do
	host_num=$(($host_num + 1))
	printf -v CONTAINER_COMMAND "fabric:container-create-ssh $JVM_CONTENT_OPTS --host ${i} --path /opt/rh/containers --user fuse --profile application application-container-0$host_num"
	ssh fuse@$ROOT_NODE "set -x;$HOST_RH_HOME/scripts/envs/$DEPLOYMENT_ENVIRONMENT/environment.sh;$HOST_RH_HOME/scripts/commands/karaf-client.sh $DEPLOYMENT_ENVIRONMENT \"$CONTAINER_COMMAND\""
        wait_for_container content-container-0$host_num
done
