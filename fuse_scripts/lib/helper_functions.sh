...
function ssh_bootstrap_fuse()
{
	...
	ssh fuse@$VAR_ROOT_NODE "set +x;$HOST_RH_HOME/scripts/commands/karaf-client.sh $VAR_ENV \"wait-for-service -t 300000 io.fabric8.api.BootstrapComplete\""
	ssh fuse@$VAR_ROOT_NODE "set +x;$HOST_RH_HOME/scripts/commands/karaf-client.sh $VAR_ENV \"fabric:create --clean --resolver manualip --manual-ip $VAR_ROOT_NODE --profile fabric --wait-for-provisioning\""
