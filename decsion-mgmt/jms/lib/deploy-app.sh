...
for i in ${APP_HOSTS[@]}; do
	host_num=$(($host_num + 1))
	printf -v CONTAINER_COMMAND "fabric:container-create-ssh $JVM_APP_OPTS --host ${i} --path /opt/rh/containers --user fuse --profile desicion-service --profile mq-client-dec-service-brokers dec-service-container-$host_num"
